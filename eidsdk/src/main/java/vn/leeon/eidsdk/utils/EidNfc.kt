...
import vn.leeon.eidsdk.jmrtd.JMRTDSecurityProvider
...
class EidNfc @Throws(GeneralSecurityException::class)
private constructor() {
    ...
    constructor(
        ...
        try {
            dg13File = getDG13File(ps, maxBlockSize)
        } catch (e: Exception) {
            e.printStackTrace()
        }
...
//            challenge
//        )
//        Log.d("TAG11", ": ${aaResult.response}")
//        Log.d(TAG, "verifyAA: "+dg15File!!.publicKey.toString())
//        Log.d(TAG, "verifyAA: "+sodFile!!.digestAlgorithm)
//        Log.d(TAG, "verifyAA: "+sodFile!!.signerInfoDigestAlgorithm)
    ...
    private fun verifyAA() {
        ...
        Log.d(TAG, "verifyAA: $dg15File $service")
        try {
    ...
    private fun verifyCS() {
        try {
            ...
            if (docSigningCertificate != null) {
                val certificateString = StringUtils.convertToBase64PEMString(docSigningCertificate)
                Log.i(TAG, "CERTIFICATE DATA:\n$certificateString")
...
//                 TODO VERIFY WITH ROOT CA ENDPOINT
//                return
            } else {
            ...
            val pkixChain = EidNfcUtils.getCertificateChain(
    ...
    private fun doEACTA(
        ...
        for (caReference in possibleCVCAReferences) {
            val eacCredentials = EidNfcUtils.getEACCredentials(caReference, cvcaKeyStores)
                ?: continue
    ...
    private fun getDG13File(ps: PassportService, maxBlockSize: Int): DG13File {
        var isDG13: InputStream? = null
        try {
            isDG13 = ps.getInputStream(PassportService.EF_DG13, maxBlockSize)
            return DG13File(isDG13)
        } finally {
            if (isDG13 != null) {
                isDG13.close()
                isDG13 = null
    ...
    companion object {
        private val TAG = EidNfc::class.java.simpleName
        ...
        public val MAX_TRANSCEIVE_LENGTH_FOR_SECURE_MESSAGING: Int =
