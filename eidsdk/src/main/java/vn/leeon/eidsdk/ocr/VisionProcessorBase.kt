...
abstract class VisionProcessorBase<T> : VisionImageProcessor<T> {
            ...
            if(isOriginalImageReturned){
                try {
                    ...
                    if (originalBitmap == null) {
                        ...
                        originalBitmap = ImageUtils.rotateBitmap(ImageUtils.getBitmap(wrap, frameMetadata)!!, frameMetadata.rotation.toFloat())
                    }
                }catch (e:Exception){
                    e.printStackTrace()
                }
    ...
    // Bitmap version
    override fun process(bitmap: Bitmap, rotation: Int, graphicOverlay: GraphicOverlay?, isOriginalImageReturned:Boolean, convertToNv21:Boolean, listener: Listener<T>):Boolean {
        if (shouldThrottle.get()) {
            return false
        }
        try{
            ...
            when(rotation){
                0 -> {
                    ...
                    bitmapToProcess = ImageUtils.rotateBitmap(bitmap, rotation.toFloat())
                }
            }
